...
class FineNet_SelfAtt_chas(nn.Module):

    def __init__(self, chas):
        super(FineNet_SelfAtt_chas, self).__init__()
        self.Conv1 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv2 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv3 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv4 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv5 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Relu = nn.ReLU()
        self.Sig = nn.Sigmoid()
...
class FineNet_SelfAtt_DCN(nn.Module):

    def __init__(self):
        super(FineNet_SelfAtt_DCN, self).__init__()
        self.Conv1 = nn.Conv2d(31, 31, 3, 1, 1)
        self.Conv2 = nn.Conv2d(31, 31, 3, 1, 1)
        self.Conv3 = nn.Conv2d(31, 31, 3, 1, 1)
        self.Conv4 = nn.Conv2d(31, 31, 3, 1, 1)
        self.Conv5 = nn.Conv2d(31, 31, 3, 1, 1)
        #self.Conv3 = DeformConv2d(31, 31, 3, 1, 1, modulation=False)
        self.Relu = nn.ReLU()
        self.Sig = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        #    y_out += self.Conv3(self.Relu(y[:,i,:,:].unsqueeze(1)))
            
        #with torch.no_grad(): 
        #    for i in range(out.shape[1]):
        #        out[:,i,:,:] = self.Conv3(self.Relu(out[:,i,:,:].unsqueeze(1)))
        ...
        return out + x#, torch.cat(y_out, 0)



class FineNet_SelfAtt_Chikusei(nn.Module):

    def __init__(self, chas):
        super(FineNet_SelfAtt_Chikusei, self).__init__()
        self.Conv1 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv2 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv3 = nn.Conv2d(chas, chas, 3, 1, 1)
        self.Conv4 = nn.Conv2d(chas, chas, 3, 1, 1)
